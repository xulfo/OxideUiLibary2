...
-- MM2 helpers: Coins are BaseParts named "Coin" under Workspace.Hotel.CoinAreas etc, Level/Coin count in leaderstats or Data
local function GetCoins()
    -- try leaderstats Coins
...
-- MM2 re-syncs WS/JP within ~50ms, so we must re-apply every Heartbeat
...
-- MM2 knocks you to the ground (state Physics) when shot/collided.
...
if not _G.ArcMm2AntiAFK then
    _G.ArcMm2AntiAFK = true
...
TpMiscSub:AddSection("MM2 Spots")
local spots = {
...
-- ══════════════════════════════════════════════════════════════════════════════
-- VISUAL TAB (ESP + world)
-- ══════════════════════════════════════════════════════════════════════════════
local VisualsTab = Window:AddTab({ Name = "Visual", Subtitle = "ESP & money", Icon = "eye" })
...
EspSub:AddToggle({ Name = "Role ESP", Default = true, Flag = "esp_role", Description = "Colors Murderer/Sheriff/Innocent", Callback = function(v) esp.roleESP = v end })
EspSub:AddToggle({ Name = "Coin ESP", Default = false, Flag = "esp_coin", Description = "Highlight coins", Callback = function(v) esp.coinESP = v end })
EspSub:AddToggle({ Name = "Gun ESP", Default = false, Flag = "esp_gun", Description = "Highlight dropped gun/knife", Callback = function(v) esp.gunESP = v end })
...
local espConn = RunService.RenderStepped:Connect(function()
    ...
    for p, obj in pairs(playerObjects) do
        ...
        if visible and head and hrp2 then
                ...
                -- Shadow color for role ESP (must be inside block to affect all drawing below)
...
-- ══════════════════════════════════════════════════════════════════════════════
-- GAMEPLAY TAB  (Coins & Roles)
-- ══════════════════════════════════════════════════════════════════════════════
local RageTab = Window:AddTab({ Name = "Gameplay", Subtitle = "Coins & roles", Icon = "target" })
local RageFarmSub = RageTab:AddSubTab("Auto Farm")
local RageHitboxSub = RageTab:AddSubTab("Roles & Hitbox")

-- Auto Farm (Coins) for MM2
RageFarmSub:AddSection("Coin Farm")
...
-- Actual coin collection loop (uses the Auto Farm toggle above)
task.spawn(function()
        local function getNearestCoin()
            ...
            for _, v in ipairs(Workspace:GetDescendants()) do
                if v:IsA("BasePart") and v.Transparency < 1 and v.Parent and (v.Name == "Coin" or v.Name:lower():find("coin")) then
                    -- ignore coins already in my character / backpack
                    if v:IsDescendantOf(LocalPlayer.Character) or v:IsDescendantOf(LocalPlayer) then
                        -- don't skip; coin might be near, but skip if actually owned? Unlikely.
                    end
        ...
        if autoCollect then
            local ok = pcall(function()
                ...
                if coin and hrp then
                    ...
                    if dist < 10 then
                        -- Robust collect: firetouchinterest + direct teleport (covers both Touched and proximity systems)
...
-- ── Rage (Murderer) ──
local RageTabMM2 = Window:AddTab({ Name = "Rage", Subtitle = "Murderer only", Icon = "skull" })
local RageMurderSub = RageTabMM2:AddSubTab("Murderer")
local RageSheriffSub = RageTabMM2:AddSubTab("Sheriff")

local murderKillAll = false
local murderKillDistance = 35
local murderKillDelay = 0.18

RageMurderSub:AddSection("Murderer")
...
local function killPlayer(targetPlr)
    ...
    -- temporarily expand hitbox for this target
...
RageMurderSub:AddToggle({
    Name = "Kill Aura (nearby)", Default = false, Flag = "mm2_killaura",
...
RageMurderSub:AddSlider({
    Name = "Aura Distance", Min = 5, Max = 80, Default = 35, Suffix = " studs", Flag = "mm2_killdist",
    Callback = function(v) murderKillDistance = v end,
})
RageMurderSub:AddSlider({
    Name = "Kill Delay", Min = 0.05, Max = 1, Default = 0.18, Suffix = "s", Flag = "mm2_killdelay",
    Callback = function(v) murderKillDelay = v end,
})
RageMurderSub:AddButton({
    Name = "Kill All (Murderer)", Primary = true,
    Callback = function()
...
RageSheriffSub:AddSection("Sheriff Helper")
RageSheriffSub:AddButton({
    Name = "Grab Gun if Dropped",
    Callback = function()
        local gunDrop = nil
        for _, v in ipairs(Workspace:GetDescendants()) do
...
-- Robust hook: try GunHandler module (MM2), then fall back to Raycast hook
do
    local G = getgenv and getgenv() or _G
    if not G.ArcSheriffHooked then
        G.ArcSheriffHooked = true
        local mod
        ...
        elseif mod and type(mod.getAim) == "function" and HookFn then
            ...
            G.ArcSheriffOrigAim = orig
            print("Sheriff: GunHandler.getAim hooked")
        else
            -- Fallback: hook Workspace.Raycast so the gun's ray is redirected
            if not G.ArcSheriffRayHooked then
                G.ArcSheriffRayHooked = true
...
RageSheriffSub:AddSection("Silent Aim")
RageSheriffSub:AddToggle({
    Name = "Silent Aim", Default = false, Flag = "sheriff_silent",
    Callback = function(v)
...
RageSheriffSub:AddDropdown({
    Name = "Target Mode", Options = {"Murderer", "FOV", "Nearest", "Chosen"}, Default = "Murderer", MaxVisible = 4, Flag = "sheriff_targetmode",
...
local chosenDropdown = RageSheriffSub:AddDropdown({
    Name = "Chosen Player", Options = GetSheriffPlayerNames(), Default = nil, MaxVisible = 6, Searchable = true, Flag = "sheriff_chosen",
    Callback = applyChosen,
})
registerResync(chosenDropdown, applyChosen)
RageSheriffSub:AddButton({ Name = "Refresh Players", Callback = function() chosenDropdown:SetOptions(GetSheriffPlayerNames()) end })
RageSheriffSub:AddSlider({ Name = "FOV Radius", Min = 10, Max = 600, Default = 150, Suffix = "px", Flag = "sheriff_fov", Callback = function(v) sheriff.fov = v end })
RageSheriffSub:AddToggle({ Name = "Show FOV", Default = true, Flag = "sheriff_showfov", Callback = function(v) sheriff.showFOV = v end })
RageSheriffSub:AddToggle({ Name = "Team Check", Default = false, Flag = "sheriff_teamcheck", Callback = function(v) sheriff.teamCheck = v end })
RageSheriffSub:AddToggle({ Name = "Wall Check", Default = false, Flag = "sheriff_wallcheck", Callback = function(v) sheriff.visibleCheck = v end })
RageSheriffSub:AddDropdown({ Name = "Hit Part", Options = {"Head", "HumanoidRootPart", "UpperTorso", "Random"}, Default = "Head", MaxVisible = 4, Flag = "sheriff_hitpart", Callback = function(v) sheriff.hitPart = v end })
RageSheriffSub:AddSlider({ Name = "Hit Chance", Min = 0, Max = 100, Default = 100, Suffix = "%", Flag = "sheriff_hitchance", Callback = function(v) sheriff.hitChance = v end })
...
-- Coin + Gun ESP highlights (throttled)
local coinHighlights = {} -- [part]=Highlight
task.spawn(function()
    while not HUB.dead do
...
local function Cleanup()
    ...
    -- remove drawings (including corner/skeleton/headCircle)
...
-- BOOT
-- ══════════════════════════════════════════════════════════════════════════════
Notify("MM2", "Arc HUB loaded — Coins: "..tostring(GetCoins()).." | Role: "..GetRole(), "Success", 3)
